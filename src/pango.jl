...
let available_font_families = Set{AbstractString}()
    ...
    function match_font(families::AbstractString, size::Float64)
        ...
        fd = ccall((:pango_font_description_from_string, libpango), Ptr{Nothing}, (Ptr{UInt8},), desc)
        return fd
    end
end
...
function PangoLayout()
    layout = ccall((:pango_layout_new, libpango),
                   Ptr{Nothing}, (Ptr{Nothing},), pango_cairo_ctx)
    # TODO: finalizer?
...
function pango_set_font(pangolayout::PangoLayout, family::AbstractString, pts::Number)
    ...
    ccall((:pango_layout_set_font_description, libpango),
          Nothing, (Ptr{Nothing}, Ptr{Nothing}), pangolayout.layout, fd)
end
...
function pango_text_extents(pangolayout::PangoLayout, text::AbstractString)
    ...
    extents = Array{Int32}(uninitialized, 4)
    ccall((:pango_layout_get_extents, libpango),
          Nothing, (Ptr{Nothing}, Ptr{Int32}, Ptr{Int32}),
...
#
function unpack_pango_attr(ptr::Ptr{Nothing}, t::Symbol)
    ptr += sizeof(Ptr{Nothing}) # skip `klass` pointer
    ptr = convert(Ptr{UInt32}, ptr)
    idx = unsafe_wrap(Array, ptr, (2,), false)
    ptr += 2 * sizeof(UInt32)
    ptr = convert(Ptr{Nothing}, ptr)
...
# Returns:
#   And int value.
unpack_pango_int(ptr::Ptr{Nothing}) = unsafe_wrap(Array, convert(Ptr{Int32}, ptr), (1,), false)[1]
unpack_pango_float(ptr::Ptr{Nothing}) = unsafe_wrap(Array, convert(Ptr{Float64}, ptr), (1,), false)[1]

#function unpack_pango_size(ptr::Ptr{Nothing})
...
#
function unpack_pango_attr_list(ptr::Ptr{Nothing})
    ...
    attr_it_next = () -> ccall((:pango_attr_iterator_next, libpango),
                               Int32, (Ptr{Nothing},), attr_it)

    attr_it_get = attr_name -> ccall((:pango_attr_iterator_get, libpango),
                                     Ptr{Nothing}, (Ptr{Nothing}, Int32),
                                     attr_it, eval(attr_name))

    attr_it_range = () -> begin
        start_idx = Array{Int32}(uninitialized, 1)
        end_idx = Array{Int32}(uninitialized, 1)
    ...
    attrs = Array{Tuple{Int, PangoAttr}}(uninitialized, 0)
...
function pango_to_svg(text::AbstractString)
    ...
    c_attr_list = Ref{Ptr{Nothing}}()

    ret = ccall((:pango_parse_markup, libpango),
                Int32, (Cstring, Int32, UInt32, Ptr{Ptr{Nothing}},
                        Ptr{Ptr{UInt8}}, Ptr{UInt32}, Ptr{Nothing}),
